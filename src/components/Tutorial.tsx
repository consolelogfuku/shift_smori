...
const STEPS: { title: string; body: string }[] = [
  ...
  { title: '営業所の休業日を登録する', body: '「シフトを組む」の STEP1 で、年末年始などの休業日をカレンダーでクリックします。土日祝は自動で休みです。' },
  { title: '有給と出勤希望日を登録する', body: 'STEP2 の表でセルをクリックし、有給 (終日)、有給 (時間休)、希望出勤日を選びます。' },
  { title: '必要な役割を日ごとに調整する', body: 'STEP3 のカレンダーで日付をクリックすると、その日だけ役割の人数を増やしたり減らしたりできます。' },
  { title: 'シフトを組んで保存する', body: 'STEP4 で実行し、結果を確認して Excel か CSV に出力します。データはタブを閉じると消えるので、終わったら「保存する」でファイルを PC に残し、次回は「読み込む」で戻します。' },
];
